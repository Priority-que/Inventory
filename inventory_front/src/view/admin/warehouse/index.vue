...
const tableData = ref<WarehousePageVO[]>([])
const total = ref(0)
...
const selectedRows = ref<WarehousePageVO[]>([])
...
const detail = ref<WarehousePageVO | null>(null)

const formRules: FormRules<WarehouseDTO> = {
...
async function loadData() {
  loading.value = true
  try {
    const result = await getWarehousePageApi(query)
...
function handleSelectionChange(rows: WarehousePageVO[]) {
...
function openEditDialog(row: WarehousePageVO) {
...
function openDetail(row: WarehousePageVO) {
  detail.value = row
...
async function handleDelete(ids: number[], confirmed = false) {
  ...
  if (!confirmed) {
    await ElMessageBox.confirm(`确定要删除选中的 ${ids.length} 个仓库吗？`, '删除仓库', {
      type: 'warning',
  ...
  <div class="page">
    <div class="page-header">
      <div>
        ...
        <p class="page-subtitle">维护仓库编码、地址和负责人信息</p>
      </div>
    ...
    <section class="content-panel">
      <div class="filter-section">
        <el-form :model="query" class="filter-grid filter-grid--3" label-position="top">
          ...
          <el-form-item label="地址">
            <el-input v-model="query.address" clearable placeholder="输入地址" @keyup.enter="handleSearch" />
          </el-form-item>
          <el-form-item label="负责人">
            <el-input v-model="query.managerName" clearable placeholder="输入负责人" @keyup.enter="handleSearch" />
          </el-form-item>
      ...
      <div class="table-wrap">
        ...
        >
          ...
          <el-table-column prop="code" label="仓库编码" min-width="130" show-overflow-tooltip />
          ...
          <el-table-column prop="managerPhone" label="联系电话" min-width="130" />
          ...
          <el-table-column label="操作" width="170" fixed="right">
            <template #default="{ row }">
              <div class="table-actions">
                ...
                <el-popconfirm title="确定删除该仓库吗？" confirm-button-text="删除" @confirm="handleDelete([row.id], true)">
                  <template #reference>
    ...
    <el-dialog
      ...
      :title="dialogMode === 'create' ? '新增仓库' : '编辑仓库'"
      width="720px"
      @closed="resetForm"
    >
      <el-form ref="formRef" v-loading="formLoading" :model="warehouseForm" :rules="formRules" label-width="96px">
        <div class="dialog-grid">
          <el-form-item label="仓库编码" prop="code">
            <el-input v-model="warehouseForm.code" placeholder="输入仓库编码" />
          </el-form-item>
          ...
          <el-form-item label="联系电话">
            <el-input v-model="warehouseForm.managerPhone" placeholder="输入联系电话" />
          </el-form-item>
          <el-form-item label="状态" prop="status">
            <el-select v-model="warehouseForm.status" placeholder="选择状态">
              <el-option
                v-for="item in enabledStatusOptions"
                :key="item.value"
                :label="item.label"
        ...
        <el-form-item label="备注">
          <el-input v-model="warehouseForm.remark" :rows="3" type="textarea" placeholder="填写备注" />
        </el-form-item>
      </el-form>
      <template #footer>
    ...
    <el-drawer v-model="detailVisible" title="仓库详情" size="520px">
      <el-descriptions v-if="detail" class="detail-descriptions" :column="1" border>
        ...
        <el-descriptions-item label="仓库编码">{{ formatEmpty(detail.code) }}</el-descriptions-item>
        <el-descriptions-item label="仓库名称">{{ formatEmpty(detail.name) }}</el-descriptions-item>
        <el-descriptions-item label="地址">{{ formatEmpty(detail.address) }}</el-descriptions-item>
        <el-descriptions-item label="负责人">{{ formatEmpty(detail.managerName) }}</el-descriptions-item>
        <el-descriptions-item label="联系电话">{{ formatEmpty(detail.managerPhone) }}</el-descriptions-item>
