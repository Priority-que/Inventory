...
public class PurchaseOrderItemServiceIpml implements PurchaseOrderItemService {
    ...
    public List<PurchaseOrderItemVO> getPurchaseOrderItemByOrderId(Long orderId) {
        List<PurchaseOrderItemVO> list = purchaseOrderItemMapper.getPurchaseOrderItemByOrderId(orderId);
        return list;
    }
    @Transactional
    @Override
    public Result updatePurchaseOrderItem(PurchaseOrderItemDTO purchaseOrderItemDTO) {
        ...
        if(purchaseOrderItemDTO.getUnitPrice() == null || purchaseOrderItemDTO.getUnitPrice().compareTo(BigDecimal.ZERO) <= 0){
            return Result.error("采购订单明细单价不能小于0！");
        }
        PurchaseOrderItem purchaseOrderItem1=new PurchaseOrderItem();
        purchaseOrderItem1.setOrderId(purchaseOrderItem.getOrderId());
        purchaseOrderItem1.setUnitPrice(purchaseOrderItemDTO.getUnitPrice());
        BigDecimal lineAmount = purchaseOrderItem.getOrderNumber()
                .multiply(purchaseOrderItemDTO.getUnitPrice())
                .setScale(2, RoundingMode.HALF_UP);
        purchaseOrderItem1.setLineAmount(lineAmount);
        purchaseOrderItem1.setRemark(purchaseOrderItemDTO.getRemark());
        purchaseOrderItem1.setId(purchaseOrderItem.getId());
        purchaseOrderItemMapper.updateById(purchaseOrderItem1);
    ...
    public PurchaseOrderItemVO getPurchaseOrderItemById(Long id) {
        PurchaseOrderItemVO purchaseOrderItemVO = purchaseOrderItemMapper.getPurchaseOrderItemById(id);
